...
<html lang="en">
    ...
    <a href="index.php"><header>MEDIFIND</header></a>
        ...
        <form class="row" action="addMedicine.php" method="get">
  <div class="col-3">
  ...
  <br><br><br><br>
  <div class="col-11">
  <button type="submit" class="btn btn-dark">Add Medicine</button></div>
</form>
<?php 
 ...
 $d=date('Y-m-d');
  $c= $_GET['pice'];
$sql="INSERT INTO `medicine_shop_buy`(`M_id`, `shop_id`, `Shop_Buy`, `Buy_date`) VALUES ((SELECT `M_id` FROM `medicine` WHERE M_name='$a'),'{$_SESSION["id"]}','$c','$d')";
...
$d=$c+$product["stocks"];
  ...
  $sql="INSERT INTO `medicine_shop`(`M_id`, `shop_id`, `stocks`) VALUES ((SELECT `M_id` FROM `medicine` WHERE M_name='$a'),'{$_SESSION["id"]}','$c')";
  $Admins=$con->query($sql);
  header("location: addMedicine.php");
